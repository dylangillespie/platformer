...
public class Player extends PhysicsObject {
	...
	public void setMini(boolean mini) {
		int tileSize = (int) this.hitbox.getWidth();
		if (mini) {
			walkSpeed = 600;

			int sideOffset = (int) (tileSize * 0.3);
			int topOffset = (int) (tileSize * 0.3);
			int bottomOffset = (int) (tileSize * 0.1);

			int newHeight = height - topOffset - bottomOffset;
			int newWidth = width - 2 * sideOffset;

			this.hitbox = new RectHitbox(this, sideOffset, topOffset, newWidth, newHeight);
		} else {
			walkSpeed = 400;
			int offset = (int) (tileSize * 0.1);
